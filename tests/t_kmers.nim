...
import unittest
from pbpkg/kmers import hash
from deques import items
...
test "dna_to_kmer":
    check kmers.dna_to_kmer("AAAA", 2).seeds.len() == 6
...
test "sorted_kmers":
    let
        ...
        k = 12
        kms: kmers.pot_t = kmers.dna_to_kmer(sq, k)
...
test "search":
    let
        ...
        k = 12
        kms: kmers.pot_t = kmers.dna_to_kmer(sq, k)
        qms: kmers.pot_t = kmers.dna_to_kmer(sq, k)
    check kmers.make_searchable(kms) == 0
    let hits = kmers.search(kms, qms)
    check deques.len(hits) == 4
    check sets.len(sets.toHashSet(seqUtils.toSeq(hits))) == 4  # 4 unique items
